...
const BACKGROUND_WINDOW_WIDTH = 1100;
const BACKGROUND_WINDOW_HEIGHT = 780;
...
async function parkAutomationWindow(windowId) {
  try {
    await chrome.windows.update(windowId, {
      focused: false,
      state: "minimized"
    });
    console.log(`[opencli] Parked automation window ${windowId} in minimized background mode`);
  } catch (err) {
    console.warn(`[opencli] Failed to minimize automation window ${windowId}:`, err);
  }
}
async function getAutomationWindow(workspace, initialUrl) {
  ...
  if (existing) {
    try {
      ...
      await parkAutomationWindow(existing.windowId);
  ...
  await parkAutomationWindow(session.windowId);
